...
func configureHost(client *SSHClient, hl *HostLogger, host string, cfg *Config, mode Mode, users []string) HostResult {
	...
	// --- Validation ---
	// NOTE: sshd is intentionally NOT restarted here, and "SSHD Running"
	// is intentionally NOT checked here. Restarting sshd on the box we're
	// currently connected to can disrupt this very SSH session/channel
	// multiplexing, causing subsequent commands on this same connection
	// to silently return empty output even though the connection isn't
	// fully dead. So the restart is deferred until after all validation
	// on this connection is complete, and sshd's post-restart health is
	// verified separately in main.go using a brand new connection.
	validations := []ValidationResult{}
	...
	// --- Restart sshd LAST, after all validation over this connection
	// is complete. Its post-restart health is checked separately in
	// main.go via verifySSHDRunning, using a fresh connection. ---
	hl.WriteLine("\n====================================================\n")
	hl.WriteLine("Restarting sshd to apply changes...\n")
	if _, err := runCmd(client, hl, "service sshd restart || systemctl restart sshd"); err != nil {
		if result.Err == nil {
			result.Err = fmt.Errorf("restarting sshd: %w", err)
		}
	}

	// NOTE: hl.WriteFooter() is intentionally NOT called here. main.go
	// appends a POST-RESTART CHECK section to this same log (via
	// verifySSHDRunning) after configureHost returns, and writes the
	// footer itself right before hl.Close().
